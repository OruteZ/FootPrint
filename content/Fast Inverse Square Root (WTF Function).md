...
I_{x}= 2^{23}E + F
$$
Bit 표기방법은 [[IEEE 부동소수점 연산 표준]] 문서 참고. (있다면...)
...
$$
이 수식에 따라서, x를 long 비트로 캐스팅해서 해당 수식을 적용한 다음, 도출된 long 비트를 다시 float으로 해석하면 근사치를 구하는 작업이 겨우 세 줄로(_캐스팅을 제외한다면 세 번의 연산으로_) 끝나게 된다.

이후 한 두번의 뉴턴/랍슨 방법 적용만으로, $\frac{1}{\sqrt x}$ 를 매우 빠르게 구할 수 있게 된다.
```cpp
y = y * ( threehalfs - ( x2 * y * y ) ); // 1st iteration
```
